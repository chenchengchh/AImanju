...
ZIMAGE_URL = _CONFIG["image_gen"]["local"]["url"]  # 本地生图走 ComfyUI（Z-Image-Turbo）
...
def _free_gpu_for_ai(server):
    """提交 GPU 任务前释放显存：卸载 Ollama 已加载模型 + 清 ComfyUI 模型缓存。

    24GB 显卡上 Ollama LLM（qwen3.6:35b ≈21GB）与视频生成（UNET ≈20GB）互斥；
    不卸载 LLM 时 ComfyUI 采样会 cudaMalloc OOM（表现为 /prompt 后报错）。
    幂等：无已加载模型时静默跳过。失败不影响提交主流程。返回释放项列表（仅提示）。"""
    freed = []
    try:
        ollama = str((_CONFIG.get("llm") or {}).get("local", {}).get("url") or "http://127.0.0.1:11434").rstrip("/")
        ps = api_get(ollama, "/api/ps", timeout=10)
        for m in (ps or {}).get("models") or []:
            name = m.get("name") or m.get("model")
            if not name:
                continue
            api_post(ollama, "/api/generate", {"model": name, "keep_alive": 0}, timeout=10)
            freed.append(f"Ollama:{name}")
    except Exception:
        pass
    try:
        api_post(server, "/free", {"unload_models": True, "free_memory": True}, timeout=15)
        freed.append("ComfyUI缓存")
    except Exception:
        pass
    return freed


def _ai_prompt(server, graph, client_id="batch_console"):
    """提交 GPU 任务前先释放显存，再 POST /prompt。统一走此函数保证不 OOM。"""
    _free_gpu_for_ai(server)
    return api_post(server, "/prompt", {"prompt": graph, "client_id": client_id})
...
def zimage_check(timeout=4):
    """检测本地 ComfyUI（Z-Image 生图）是否在线，并列出可用的 Z-Image 主模型。"""
    try:
        api_get(ZIMAGE_URL, "/system_stats", timeout=timeout)
    except Exception as e:
        return {"ok": False, "error": str(e)}
    unets, _ = _server_models(ZIMAGE_URL)
    zmodels = [m for m in unets if "z_image" in str(m).lower()]
    return {"ok": True, "models": zmodels or unets}
...
def zimage_generate(prompt, filename, size="768x1024", timeout=300):
    """文生图统一入口：本地 ComfyUI Z-Image 优先，配置云端或本地失败时降级云端。"""
    ...
    # 本地 ComfyUI Z-Image
    try:
        return _zimage_local(prompt, filename, size, timeout)
    except Exception as e:
        last_err = e
...
def _zimage_local(prompt, filename, size="768x1024", timeout=300):
    """通过 ComfyUI Z-Image-Turbo 工作流文生图，图片保存到 素材/ 目录。返回 (filename, 绝对路径)。

    工作流与 F:\\2026-ComfyUI-V8.3\\blueprints\\Text to Image (Z-Image-Turbo).json 一致（API 格式）：
    UNETLoader(z_image_turbo_bf16) -> ModelSamplingAuraFlow(shift=3) -> KSampler(8步, cfg=1, res_multistep/simple)
    -> VAEDecode(ae.safetensors) -> SaveImage；CLIP 用 qwen_3_4b(lumina2)，negative 用 ConditioningZeroOut。
    """
    cfg = _CONFIG["image_gen"]["local"]
    try:
    ...
    graph = {
        "28": {"class_type": "UNETLoader", "inputs": {
            "unet_name": str(cfg.get("unet") or "z_image_turbo_bf16.safetensors"),
            "weight_dtype": "default"}},
        "30": {"class_type": "CLIPLoader", "inputs": {
            "clip_name": str(cfg.get("clip") or "qwen_3_4b.safetensors"),
            "type": "lumina2", "device": "default"}},
        "29": {"class_type": "VAELoader", "inputs": {
            "vae_name": str(cfg.get("vae") or "ae.safetensors")}},
        ...
        "3": {"class_type": "KSampler", "inputs": {
            ...
            "steps": int(cfg.get("steps") or 8), "cfg": 1.0,
            "sampler_name": "res_multistep", "scheduler": "simple", "denoise": 1.0}},
        "8": {"class_type": "VAEDecode", "inputs": {"samples": ["3", 0], "vae": ["29", 0]}},
        "9": {"class_type": "SaveImage", "inputs": {"images": ["8", 0], "filename_prefix": "zimage_console"}},
    }
    resp = _ai_prompt(ZIMAGE_URL, graph)
    ...
    while time.time() < deadline:
        time.sleep(2)
        hist = api_get(ZIMAGE_URL, f"/history/{pid}", timeout=15)
        item = hist.get(pid)
        if not item:
            continue
        status = item.get("status") or {}
        if status.get("status_str") == "error":
            raise RuntimeError(f"ComfyUI 生图执行失败：{status.get('messages')}")
        imgs = (item.get("outputs", {}).get("9") or {}).get("images") or []
        if imgs:
            o = imgs[0]
            q = urllib.parse.urlencode({
            ...
            req = urllib.request.Request(ZIMAGE_URL + "/view?" + q, headers={"User-Agent": "batch-console"})
            with _opener().open(req, timeout=120) as r:
                raw = r.read()
            ...
            with open(dest, "wb") as f:
                f.write(raw)
            return filename, dest
    raise RuntimeError(f"Z-Image 生图超时（{timeout}s）")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.path == "/api/config":
            ...
            return
        if path.path == "/api/zimage_check":
            self._send(200, json.dumps(zimage_check(), ensure_ascii=False))
            return
    ...
    def do_POST(self):
        ...
        if path == "/api/zimage_gen":
            ...
            if not prompt.strip() or not filename.strip():
                ...
                return
            try:
                fn, dest = zimage_generate(prompt, filename, size)
            except Exception as e:
                self._send(400, json.dumps({"error": f"Z-Image 生图失败：{e}"}, ensure_ascii=False))
                return
        ...
        if path == "/api/asset_gen":
            ...
            while attempts < 3:
                try:
                    fn, dest = zimage_generate(prompt, filename, "768x1024")
                except Exception as e:
                    self._send(400, json.dumps({"error": f"Z-Image 生图失败：{e}"}, ensure_ascii=False))
                    return
                attempts += 1
                # 一键生成跳过质检（35B 质检每张约 2.5 分钟，连续生成易拖垮浏览器）；
                # 质检可之后用「补质检」按钮单独做。
                if skip_verify:
                    v = {"ok": True, "issues": []}
                else:
